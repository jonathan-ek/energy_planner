...
from .utils import update_entities
...
def parse_datetime(val, zone):
    if type(val) is str:
        return dt.datetime.fromisoformat(val).astimezone(zone)
    return val.astimezone(zone)


async def plan_day(hass: HomeAssistant, nordpool_values: [dict], config: dict):
    _LOGGER.info("plan_day: %s", nordpool_values)
    charge_hours = [x for x in nordpool_values if config['earliest_charge'] <= x['start'] < config['earliest_discharge']]
    discharge_hours = [x for x in nordpool_values if config['earliest_discharge'] <= x['start']]
    cheapest_hours = sorted(
        sorted(charge_hours, key=lambda x: x['value'])[:config['nr_of_charge_hours']],
        key=lambda x: x['start']
    )
    expensive_hours = sorted(
        sorted(discharge_hours, key=lambda x: x['value'], reverse=True)[:config['nr_of_discharge_hours']],
        key=lambda x: x['start']
    )
    ...
    for i, hour in enumerate(expensive_hours):
        if i == 0:
            ...
            schedule.append({**hour, "state": "discharge"})
        else:
            if schedule[-1]['end'] == hour['start']:
                schedule[-1]['end'] = hour['end']
            else:
    ...
    if len(schedule) > 0:
        hass.data[DOMAIN]['values'][f"slot_{index + len(schedule)}_date_time_start"] = schedule[-1]['end']
...
async def planner(hass: HomeAssistant, *args, **kwargs):
    ...
    start_of_day = (now - dt.timedelta(days=1)).astimezone(zone).replace(
        hour=earliest_charge.hour, minute=earliest_charge.minute, second=0, microsecond=0
    )
    start_of_discharge = now.astimezone(zone).replace(
        hour=earliest_discharge.hour, minute=earliest_discharge.minute, second=0, microsecond=0
    )
    ...
    today_data = [{
        ...
        "value": x['value']
    } for x in [*yesterday, *today] if start_of_day <= parse_datetime(x['start'], zone) < end_of_day]
    ...
    if tomorrow is not None:
        ...
        start_of_discharge = (now + dt.timedelta(days=1)).astimezone(zone).replace(
            hour=earliest_discharge.hour, minute=earliest_discharge.minute, second=0, microsecond=0
        )
        end_of_day = (now + dt.timedelta(days=1)).astimezone(zone).replace(
            hour=earliest_charge.hour, minute=earliest_charge.minute, second=0, microsecond=0
        )
        config = {
            "earliest_charge": start_of_day,
        ...
        tomorrow_data = [{
            ...
            "end": parse_datetime(x['end'], zone),
            "value": x['value']
        } for x in [*today, *tomorrow] if
            start_of_day <= parse_datetime(x['start'], zone) < end_of_day]
